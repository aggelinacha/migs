...
def render(
    ...
    _xyz = pc.get_xyz[:, data.identity_idx] if multigaussian else pc.get_xyz
    ...
    means3D = pc.get_xyz[:, data.identity_idx] if multigaussian else pc.get_xyz
    means2D = screenspace_points
    opacity = pc.get_opacity[:, data.identity_idx] if multigaussian else pc.get_opacity
    ...
    if pipe.compute_cov3D_python:
        cov3D_precomp = pc.get_covariance(data.identity_idx, scaling_modifier) if multigaussian else pc.get_covariance(scaling_modifier)
    else:
        scales = pc.get_scaling[:, data.identity_idx] if multigaussian else pc.get_scaling
        rotations = pc.get_rotation[:, data.identity_idx] if multigaussian else pc.get_rotation
