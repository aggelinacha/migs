...
class SkinningField(RigidDeform):
    def __init__(self, cfg, metadata, subjects=None, multigaussian=False):
        ...
        if subjects is None:
            ...
            for subject in subjects:
                self.smpl_verts[subject] = metadata[subject]['smpl_verts']
                self.skinning_weights[subject] = metadata[subject]['skinning_weights']
                self.aabb[subject] = metadata[subject]['aabb']
                self.cano_mesh[subject] = metadata[subject]['cano_mesh']
    ...
    def sample_skinning_loss(self, subject=None):
        smpl_verts = self.smpl_verts if subject is None else self.smpl_verts[subject]
        skinning_weights = self.skinning_weights if subject is None else self.skinning_weights[subject]
        cano_mesh = self.cano_mesh if subject is None else self.cano_mesh[subject]
        ...
        pts_W = (skinning_weights[vert_ids] * bary_coords[..., None]).sum(axis=1)
    ...
    def get_skinning_loss(self, subject=None):
        pts_skinning, sampled_weights = self.sample_skinning_loss(subject=subject)
        aabb = self.aabb if subject is None else self.aabb[subject]
    ...
    def forward(self, gaussians, iteration, camera):
        ...
        n_pts = xyz.shape[0]
        aabb = self.aabb if subject is None else self.aabb[subject].to(xyz.device)
    ...
    def regularization(self, subject=None):
        loss_skinning = self.get_skinning_loss(subject=subject)
        return {
            'loss_skinning': loss_skinning
        }

def get_rigid_deform(cfg, metadata, subjects=None, multigaussian=False):
    name = cfg.name
    model_dict = {
        "identity": Identity,
        "smpl_nn": SMPLNN,
