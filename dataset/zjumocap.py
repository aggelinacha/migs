...
class ZJUMoCapDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        if split == 'predict':
            predict_seqs = ['gBR_sBM_cAll_d04_mBR1_ch05_view1',
                            ...
                            'gJB_sFM_cAll_d08_mJB4_ch12_view1',
                            '../CoreView_377/models',
                            '../CoreView_386/models',
                            '../CoreView_387/models',
                            '../CoreView_392/models',
            ...
            if isinstance(predict_seq, int):
                ...
                self.subject = predict_seq
                predict_seq = "models" #"canonical_pose_view1" #"models"
            if "CoreView" in predict_seq:
                self.subject = predict_seq.replace("../", "").replace("/models", "") 
        ...
        self.H, self.W = cfg.img_HW #1024, 1024 # hardcoded original size
        self.h, self.w = cfg.img_hw

        self.faces = np.load(os.path.join(self.cfg.smpl_data_path, "misc/faces.npz"))['faces']
        self.skinning_weights = dict(np.load(os.path.join(self.cfg.smpl_data_path, "misc/skinning_weights_all.npz")))
        self.posedirs = dict(np.load(os.path.join(self.cfg.smpl_data_path, "misc/posedirs_all.npz")))
        self.J_regressor = dict(np.load(os.path.join(self.cfg.smpl_data_path, "misc/J_regressors.npz")))
        ...
        if cfg.freeview:
            ...
            self.cameras = freeview_camera(self.cameras[cam_names[0]], trans,  rotate_axis='y', total_frames=100)
            cam_names = self.cameras['all_cam_names']
    ...
    def getitem(self, idx, data_dict=None):
        ...
        if "CoreView" in img_file:
            ...
            T = model_dict['t'].astype(np.float32)[..., None]
        R = M @ R
        T = M @ T
...
class ZJUMoCapDatasetMulti(Dataset):
    def __init__(self, cfg, split='train'):
        super().__init__()
        self.cfg = cfg
        self.split = split
        ...
        self.subjects = list(sorted(cfg.subjects))

        assert isinstance(self.subjects, list), "Input subject(s) must be a list"

        self.test_subject = None
        self.test_identity_idx = None
        if hasattr(cfg, "test_subject"):
            self.test_subject = cfg.test_subject
        ...
        if split == 'predict':
            ...
            if isinstance(predict_seq, int):
                ...
                self.test_identity_idx = self.subjects.index(self.test_subject)
                self.test_subject = predict_seq
                predict_seq = "models"
                if self.test_subject not in self.subjects:
                    self.subjects.append(self.test_subject)
            if "CoreView" in predict_seq:
                self.test_identity_idx = self.subjects.index(self.test_subject)
                self.test_subject = predict_seq.replace("../", "").replace("/models", "")
        ...
        self.val_cams = cfg.val_views
        self.white_bg = cfg.white_background
        self.H, self.W = cfg.img_HW #1024, 1024
        self.h, self.w = cfg.img_hw
        ...
        if split == 'train':
            cam_names = self.train_cams
            _frames = self.train_frames
        elif split == 'val':
            cam_names = self.val_cams
            _frames = self.val_frames
        elif split == 'test':
            cam_names = self.cfg.test_views[self.cfg.test_mode]
            _frames = self.cfg.test_frames[self.cfg.test_mode]
        elif split == 'predict':
            cam_names = self.cfg.predict_views
            _frames = self.cfg.predict_frames
        else:
            raise ValueError
        ...
        self.cameras = {}
        self.model_files = {}
        self.model_files_list = []
        self.data = {}
        self.frames = {}
        self.data_list = []
        for identity_idx, subject in enumerate(self.subjects):
            with open(os.path.join(self.root_dir, subject, 'cam_params.json'), 'r') as f:
                self.cameras[subject] = json.load(f)

            if len(cam_names) == 0:
                cam_names = self.cameras[subject]['all_cam_names']
            elif self.refine:
                cam_names = [f'{int(cam_name) - 1:02d}' for cam_name in cam_names]

            start_frame, end_frame, sampling_rate = _frames

            subject_dir = os.path.join(self.root_dir, subject)
            if split == 'predict':
                model_files = sorted(glob.glob(os.path.join(subject_dir, predict_seq, '*.npz')))
                self.model_files[subject] = model_files
                frames = list(reversed(range(-len(model_files), 0)))
                if end_frame == 0:
                    end_frame = len(model_files)
                frame_slice = slice(start_frame, end_frame, sampling_rate)
            ...
            if cfg.freeview:
                ...
                model_dict = np.load(model_files[0])
                trans = model_dict['trans'].astype(np.float32)
                self.cameras[subject] = freeview_camera(self.cameras[subject][cam_names[0]], trans, rotate_axis='y', total_frames=10)
                cam_names = self.cameras[subject]['all_cam_names']
            ...
            if split == 'predict' or cfg.freeview:
                for cam_idx, cam_name in enumerate(cam_names):
                    ...
                    for d_idx, f_idx in enumerate(frames):
                        ...
                        if self.test_subject is None or subject == self.test_subject:
                            self.data_list.append({
                                "subject": subject,
                                "data_idx": d_idx,
                                "frame_idx": f_idx,
                                "identity_idx": self.test_identity_idx if self.test_identity_idx is not None else identity_idx,
                            })
            else:
                for cam_idx, cam_name in enumerate(cam_names):
                    cam_dir = os.path.join(subject_dir, cam_name)
            ...
            self.frames[subject] = frames
            self.model_files_list.extend(model_files)
    ...
    def get_metadata(self):
        self.metadata = {}
        self.metadata["frame_dict"] = {}

        for subject in self.subjects:
            data_paths = self.model_files[subject]
            if len(data_paths) == 0: continue

            cano_data = self.get_cano_smpl_verts(data_paths[0])
            
            self.metadata[subject] = cano_data

            if self.split != 'train':    
                continue

            step = self.train_frames[-1]
            frames = list(range(len(data_paths)))
            start = 0
            end = len(frames)
            frame_slice = slice(start, end, step)
            ...
            self.metadata["frame_dict"][subject] = frame_dict
    
            if self.cfg.train_smpl:
                self.metadata[subject].update(self.get_smpl_data(subject=subject))
    ...
    def get_smpl_data(self, subject):
        # load all smpl fitting of the training sequence
        if self.split != 'train':
            return {}
        ...
        for idx, (frame, model_file) in enumerate(zip(self.frames[subject], self.model_files[subject])):
            model_dict = np.load(model_file)
    ...
    def getitem(self, idx, data_dict=None):
        if data_dict is None:
            subject, idx_s = self.data_list[idx]["subject"], self.data_list[idx]["data_idx"]
            data_dict = self.data[subject][idx_s]
        cam_idx = data_dict['cam_idx']
        cam_name = data_dict['cam_name']
        data_idx = data_dict['data_idx']
        img_file = data_dict['img_file']
        mask_file = data_dict['mask_file']
        model_file = data_dict['model_file']
        frame_idx = data_dict['frame_idx']   # self.metadata['frame_dict'][idx // len(self.cam_names)]  # data_dict['frame_idx']
        identity_idx = self.data_list[idx]['identity_idx']

        K = np.array(self.cameras[subject][cam_name]['K'], dtype=np.float32).copy()
        if "CoreView" in img_file:
            dist = np.array(self.cameras[subject][cam_name]['D'], dtype=np.float32).ravel()
        R = np.array(self.cameras[subject][cam_name]['R'], np.float32)
        T = np.array(self.cameras[subject][cam_name]['T'], np.float32)

        model_dict = np.load(model_file)
        
        # For test
        if identity_idx != self.subjects.index(subject):
            subject = self.subjects[identity_idx]
        ...
        # Compute posed SMPL body
        minimal_shape = self.metadata[subject]['minimal_shape']
        gender = self.metadata[subject]['gender']
        ...
        # final bone transforms that transforms the canonical Vitruvian-pose mesh to the posed mesh
        # without global translation
        bone_transforms_02v = self.metadata[subject]['bone_transforms_02v']
        bone_transforms = bone_transforms @ np.linalg.inv(bone_transforms_02v)
        bone_transforms = bone_transforms.astype(np.float32)
        bone_transforms[:, :3, 3] += trans  # add global offset
    ...
    def readPointCloud(self, subject=None):
        if subject is None:
            subject = self.subjects[0]
        if self.cfg.get('random_init', False):
            ply_path = os.path.join(self.root_dir, subject, 'random_pc.ply')
            ...
            ply_path = os.path.join(self.root_dir, subject, 'cano_smpl.ply')
            try:
                pcd = fetchPly(ply_path)
            except:
                verts = self.metadata[subject]['smpl_verts']
                faces = self.faces
                mesh = trimesh.Trimesh(vertices=verts, faces=faces)
                n_points = 50_000
