...
class NoPoseCorrection(nn.Module):
    def __init__(self, config, metadata=None, subjects=None):
...
class PoseCorrection(nn.Module):
    def __init__(self, config, metadata=None):
        ...
        self.data_list = metadata['data_list'] if 'data_list' in metadata else None

        gender = metadata['gender']

        v_template = np.load(os.path.join(self.config.smpl_data_path, "misc/v_templates.npz"))[gender]
        lbs_weights = np.load(os.path.join(self.config.smpl_data_path, "misc/skinning_weights_all.npz"))[gender]
        posedirs = np.load(os.path.join(self.config.smpl_data_path, "misc/posedirs_all.npz"))[gender]
        posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
        shapedirs = np.load(os.path.join(self.config.smpl_data_path, "misc/shapedirs_all.npz"))[gender]
        J_regressor = np.load(os.path.join(self.config.smpl_data_path, "misc/J_regressors.npz"))[gender]
        kintree_table = np.load(os.path.join(self.config.smpl_data_path, "misc/kintree_table.npy"))
...
class DirectPoseOptimization(PoseCorrection):
    def __init__(self, config, metadata=None, subjects=None):
        ...
        if subjects is None:
            ...
            for subject in subjects:
                root_orient.append(np.array(metadata[subject]['root_orient']))
                pose_body.append(np.array(metadata[subject]['pose_body']))
                pose_hand.append(np.array(metadata[subject]['pose_hand']))
                trans.append(np.array(metadata[subject]['trans']))
                betas.append(metadata[subject]['betas'])
            root_orient = np.concatenate(root_orient, axis=0)
            pose_body = np.concatenate(pose_body, axis=0)
            pose_hand = np.concatenate(pose_hand, axis=0)
        ...
        self.register_parameter('betas', nn.Parameter(torch.from_numpy(betas).float()))
    ...
    def pose_correct(self, camera, iteration):
        ...
        if self.data_list is not None:
            subject_idx = torch.Tensor([self.data_list[frame]["identity_idx"]]).long().to(self.betas.device)
            betas = self.betas[subject_idx]
        else:
            betas = self.betas
...
def get_pose_correction(cfg, metadata, subjects=None):
    name = cfg.name
    model_dict = {
        "none": NoPoseCorrection,
        "direct": DirectPoseOptimization,
    }
    return model_dict[name](cfg, metadata, subjects)
