...
def full_aiap_loss_multi(gs_can, gs_obs, identity_idx, n_neighbors=5):
    xyz_can = gs_can.get_xyz[:, identity_idx]
    xyz_obs = gs_obs.get_xyz[:, identity_idx]

    cov_can = gs_can.get_covariance(identity_idx)
    cov_obs = gs_obs.get_covariance(identity_idx)
