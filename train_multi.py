...
from utils.loss_utils import full_aiap_loss_multi
...
def training(config):
    ...
    first_iter = 0
    gaussians = MultiGaussianModel(model.gaussian, subjects=dataset.subjects)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        for param_group in scene.converter.optimizer.param_groups:
            if iteration % 1000 == 0:
                if "name" in param_group:
                    ...
                    print(param_group['lr'], param_group)
        # print(gaussians.U1[0])
        # print(gaussians.U2[0])
        # print(gaussians.U3[0])
        # print(gaussians._xyz[0])
        if torch.isnan(gaussians._xyz).sum() > 0:
            print("NaN", torch.isnan(gaussians._xyz).sum())
        ...
        render_pkg = render(data, iteration, scene, pipe, background, compute_loss=True, return_opacity=use_mask, multigaussian=True)
        ...
        if lambda_skinning > 0:
            loss_skinning = scene.get_skinning_loss(subject=data.subject)
        ...
        if lambda_aiap_xyz > 0. or lambda_aiap_cov > 0.:
            loss_aiap_xyz, loss_aiap_cov = full_aiap_loss_multi(scene.gaussians, render_pkg["deformed_gaussian"], identity_idx=data.identity_idx)
        else:
...
def validation(iteration, testing_iterations, testing_interval, scene : MultiScene, evaluator, renderArgs):
    ...
    for config in validation_configs:
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, data_idx in enumerate(config['cameras']):
                ...
                render_pkg = render(data, iteration, scene, *renderArgs, compute_loss=False, return_opacity=True, multigaussian=True)
                ...
                subject = "_" + data.subject if hasattr(data, "subject") and data.subject is not None else ""
                wandb_img = wandb.Image(opacity_image[None],
                                        caption=config['name'] + "_view_{}{}/render_opacity".format(data.image_name, subject))
                examples.append(wandb_img)
                wandb_img = wandb.Image(image[None], caption=config['name'] + "_view_{}{}/render".format(data.image_name, subject))
                examples.append(wandb_img)
                wandb_img = wandb.Image(gt_image[None], caption=config['name'] + "_view_{}{}/ground_truth".format(
                    data.image_name, subject))
                examples.append(wandb_img)
...
def main(config):
    ...
    wandb.init(
        ...
        name=wandb_name,
        project='gaussian-splatting-avatar-multi',
     #   entity='fast-avatar',
        dir=config.exp_dir,
