...
class GaussianConverter(nn.Module):
    def __init__(self, cfg, metadata):
        ...
        subjects = self.metadata["subjects"] if "subjects" in self.metadata else None

        self.multigaussian = cfg.model.multigaussian if hasattr(cfg.model, "multigaussian") else False

        self.pose_correction = get_pose_correction(cfg.model.pose_correction, metadata, subjects=subjects)
        self.deformer = get_deformer(cfg.model.deformer, metadata, subjects=subjects, multigaussian=self.multigaussian)
        self.texture = get_texture(cfg.model.texture, metadata, subjects=subjects, multigaussian=self.multigaussian)
    ...
    def set_optimizer(self):
        opt_params = [
            {'params': self.deformer.rigid.parameters(), 'lr': self.cfg.opt.get('rigid_lr', 0.), "name": "rigid"},
            ...
            {'params': [p for n, p in self.deformer.non_rigid.named_parameters() if 'latent' not in n],
                'lr': self.cfg.opt.get('non_rigid_lr', 0.), "name": "non_rigid"},
            ...
            {'params': self.pose_correction.parameters(), 'lr': self.cfg.opt.get('pose_correction_lr', 0.), "name": "non_rigid_lat"},
            {'params': [p for n, p in self.texture.named_parameters() if 'latent' not in n],
                'lr': self.cfg.opt.get('texture_lr', 0.), "name": "texture"},
            {'params': [p for n, p in self.texture.named_parameters() if 'latent' in n],
                'lr': self.cfg.opt.get('tex_latent_lr', 0.), 'weight_decay': self.cfg.opt.get('latent_weight_decay', 0.05), "name": "texture_lat"},
        ]
    ...
    def forward(self, gaussians, camera, iteration, compute_loss=True):
        ...
        if self.multigaussian:
            gaussians.update_attributes(camera.identity_idx)
