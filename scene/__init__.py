...
from scene.gaussian_model_multi import MultiGaussianModel
...
class Scene:
    ...
    def get_skinning_loss(self, subject=None):
        loss_reg = self.converter.deformer.rigid.regularization(subject=subject)
    ...
    def load_checkpoint(self, path):
        ...
        if "texture.latent.weight" in converter_sd:
            if converter_sd["texture.latent.weight"].shape[0] < len(self.metadata["frames"]):
                converter_sd["texture.latent.weight"] = converter_sd["texture.latent.weight"].repeat(3, 1)[:len(self.metadata["frames"])]
            elif converter_sd["texture.latent.weight"].shape[0] > len(self.metadata["frames"]):
                converter_sd["texture.latent.weight"] = converter_sd["texture.latent.weight"][:len(self.metadata["frames"])]
        converter_sd = {k: v for k, v in converter_sd.items() if "pose_correction" not in k}
...
class MultiScene:
    ...
    def __init__(self, cfg, gaussians : MultiGaussianModel, save_dir : str):
        """b
        :param path: Path to colmap scene main folder.
        """
        self.cfg = cfg
