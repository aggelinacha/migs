...
class Evaluator(nn.Module):
    ...
    def forward(self, inputs, targets, mask=None):
        psnr = self.psnr(inputs, targets, valid_mask=mask)
        ssim = self.ssim(inputs, targets, valid_mask=mask)
        lpips_ = self.lpips(inputs, targets, valid_mask=mask)
        return {
            "psnr": psnr,
            "ssim": ssim,
            "lpips": lpips_,
...
class PSNR(nn.Module):
    ...
    def forward(self, inputs, targets, valid_mask=None, reduction='mean'):
        ...
        if valid_mask is not None:
            valid_mask = valid_mask.cpu().numpy() * 255.
            ...
            if w == 0 or h == 0:
                ...
                img_pred = inputs[:, y - 100:y + h + 100, x - 100:x + w + 100]
                img_gt = targets[:, y - 100:y + h + 100, x - 100:x + w + 100]
            value = (img_pred - img_gt) ** 2
...
class SSIM(nn.Module):
    ...
    def forward(self, inputs, targets, valid_mask=None, reduction='mean'):
        ...
        # compute ssim
        ssim = compute_ssim(img_pred, img_gt, data_range=img_gt.max() - img_gt.min(), channel_axis=0)
